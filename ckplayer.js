...
function ckplayerConfig() {
    var timeScheduleAdjust=1;
    if(typeof (attrset)!='undefined'&&!attrset.ifCanDrag){
        //是否可拖拽  影响参数为19  36
        timeScheduleAdjust=1;
    }
    if(attrset.showProgressBar=="0"){
        timeScheduleAdjust=0;
    }
    var playbackRate=false;
    if(typeof(attrset)!='undefined'&&attrset.ifCanDrag){
        playbackRate=true;
    }
    if(attrset.playbackRate){
        playbackRate=true;
    }
	return {
		...
		config: {
            ...
            playbackRate:playbackRate,//是否使用倍速播放组件
			...
			timeScheduleAdjust: timeScheduleAdjust,//是否可调节调节栏,0不启用，1是启用，2是只能前进（向右拖动），3是只能后退，4是只能前进但能回到第一次拖动时的位置，5是看过的地方可以随意拖动
...
! (function() {
    ...
    var ckplayer = function(obj) {
        ...
        */
		...
		this.playbackRateArr = [[0.5, '0.5倍'], [1, '正常'], [1.25, '1.25倍'], [1.5, '1.5倍'], [2, '2倍速']];
		//全局变量：保存倍速
		...
		*/
		definition: function() {
            ...
            for (i = 0; i < vArr.length; i++) {
				...
				if(typeof(globalSrcIndex)!='undefined'&&globalSrcIndex==i){
					nowD=d;
				}
			}
			if (!nowD) {
		...
		newDefinition: function(title) {
			...
			for (var i = 0; i < vArr.length; i++) {
				...
				if (v[2] == title) {
					nVArr.push(v);
                    srcIndex=i;
					this.sendJS('definitionChange', title);
				}
			}
			if (nVArr.length < 1) {
				return;
			...
			if (!this.isM3u8) {
                ...
                globalSrcIndex=srcIndex;
				this.embedHls(vArr[srcIndex][0], this.vars['autoplay']);
			}
